...
with tab1:
    st.subheader("📊 إحصائيات المدن المتواجدة داخل نطاق التأثير")
    # بيانات تجريبية بناءً على التحليل اللي عملناه الصبح
    data = {
        'المدينة': ['عمان', 'القدس', 'تل أبيب', 'بئر السبع', 'الخليل'],
        'الدولة': ['الأردن', 'فلسطين', 'إسرائيل', 'إسرائيل', 'فلسطين'],
        'المسافة من المركز (كم)': [115, 75, 95, 25, 55],
        'مستوى الخطر': ['متوسط', 'عالي', 'عالي', 'حرج', 'عالي']
    }
    import pandas as pd
    df = pd.DataFrame(data)
    st.table(df) # بيعرض جدول شيك جداً ومقروء
with tab2:
    st.info("هذا المشروع يهدف إلى استخدام تقنيات الـ GIS وبايثون لنمذجة سيناريوهات المخاطر الجغرافية.")
# إضافة ماركرز للمدن المتأثرة
cities = {
